...
REQUIRED VARIABLES
  ...
  # wide area network. If this is empty then an isolated subnet will be created.
  wan_nic="wlan0"
...
function launch_subnet()
{
  ...
  if [[ -z $wan_nic ]] && [[ $action == up ]]
  then
    echo "warning: wan_nic is undefined" >&2
#     exit 1
  fi

  if [[ -z $subnet_nic ]]
  then
    echo "error: subnet_nic is undefined" >&2
    exit 1
  fi

  if [[ -z $mask ]]
  then
    echo "error: mask is undefined" >&2
    exit 1
  fi

  if [[ -z $subnet_ip ]]
  then
    echo "error: subnet_ip is undefined" >&2
    exit 1
  fi

  if [[ -z $server_ip ]]
  then
    echo "error: server_ip is undefined" >&2
    exit 1
  fi

  if [[ -z $iptables ]]
  then
    echo "error: iptables is undefined" >&2
    exit 1
  fi

  if [[ -z $dhcpd_conf ]]
  then
    echo "error: dhcpd_conf is undefined" >&2
    exit 1
  fi

  if [[ -z $dhcpd_lease ]]
  then
    echo "error: dhcpd_lease is undefined" >&2
    exit 1
  fi

  if [[ -z $dhcpd_pid ]]
  then
    echo "error: dhcpd_pid is undefined" >&2
    exit 1
  fi
  ...
  case "$action" in
    up)
      ...
      ip addr replace "$server_ip" dev "$subnet_nic"
